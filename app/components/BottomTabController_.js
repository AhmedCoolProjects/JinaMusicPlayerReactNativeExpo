...
export default function BottomTabController_({ navigation }) {
  const { isOn, setIsOn, lastSong } = useContext(myContext);
  return (
    <View style={styles.container}>
      <SimpleLineIcons
        onPress={() => navigation.navigate("List Music")}
        style={styles.icons_1}
        name="playlist"
      />
      ...
      <AntDesign
        onPress={() =>
          navigation.navigate("Played Room", {
            lastSong: lastSong,
          })
        }
        name="up"
        style={styles.icons_2}
      />
    </View>
  );
}
export function BottomTabControllerIntegrated() {
  const { isOn, setIsOn } = useContext(myContext);
  return (
    <View style={styles.containerIntegrated}>
      <AntDesign style={styles.icons_} name="leftcircleo" />
      {isOn ? (
        <Feather
          onPress={() => setIsOn(false)}
